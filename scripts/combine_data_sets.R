...
get_combined_dataset <- function(file_path){
  # Read in the combined data set
  wb_df = read_delim(file_path, delim = '\t')
}

plot_global_trends <- function(){
  # Plot all three world bank variables against year
  p = wb_df %>%
...
plot_unusual_gdp_growth <- function(){
  # Plot the distribution of GDP fold-changes and 
  # identify outliers
  
  # Calculate GDP start and end, normalized to start (so that end result is a fold-change)
  wb_norm_df = wb_df %>%
    select(-c('Indicator.Name')) %>%
...
# Which countries have far more or far fewer seniors than expected based on GDP?
mean_resid_gdp = mean(gdp_by_p65$lresid, na.rm=T)
sd_resid_gdp = sd(gdp_by_p65$lresid, na.rm=T)
outlier_thresh_gdp = mean_resid_gdp - 3*sd_resid_gdp
fewer_seniors_gdp = gdp_by_p65 %>% filter(lresid < outlier_thresh_gdp) %>% mutate(sen='few')
more_seniors_gdp = gdp_by_p65 %>% arrange(lresid) %>% top_n(5) %>% mutate(sen='more')
unusual_num_seniors_gdp = rbind(fewer_seniors_gdp, more_seniors_gdp)

p3 = ggplot(gdp_by_p65, aes(x=lresid)) +
  geom_histogram(alpha=0.6, fill='#99badd') +
  geom_vline(xintercept = mean_resid_gdp) +
  geom_vline(xintercept = outlier_thresh_gdp, color='red', alpha=0.4) +
  theme_minimal() +
  coord_flip()
print(p3)
...
mean_resid_br = mean(br_by_p65$lresid, na.rm=T)
sd_resid_br = sd(br_by_p65$lresid, na.rm=T)
outlier_thresh_br = mean_resid_br - 3*sd_resid_br
fewer_seniors_br = br_by_p65 %>% filter(lresid < outlier_thresh_br) %>% mutate(sen='few')
more_seniors_br = br_by_p65 %>% arrange(lresid) %>% top_n(5) %>% mutate(sen='more')
...
p2 = ggplot(br_by_p65, aes(x=lresid)) +
  geom_histogram(alpha=0.6, fill='#99badd') +
...
# Compare birth rate over time in nations with extremely high and low senior populations
wb_df %>% filter(Country.Name %in% unusual_num_seniors$Country.Name) %>%
  left_join(unusual_num_seniors %>% select(Country.Name, sen), by='Country.Name') %>%
  spread(key=Indicator.Code, value=Value) %>%
...
ggsave('results/br_over_time.png', height=5, width=7)
...
ggplot(world_map, aes(long, lat, group=group)) +
  geom_polygon(fill="gray", colour="darkgray", size=1) +
...
ggsave('results/world_map.png', heigh=5, width=7)
